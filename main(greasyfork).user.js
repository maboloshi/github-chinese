...
// @require      https://cn-greasyfork.org/scripts/435207-github-%E4%B8%AD%E6%96%87%E5%8C%96%E6%8F%92%E4%BB%B6-%E4%B8%AD%E6%96%87%E8%AF%8D%E5%BA%93%E8%A7%84%E5%88%99/code/GitHub%20%E4%B8%AD%E6%96%87%E5%8C%96%E6%8F%92%E4%BB%B6%20-%20%E4%B8%AD%E6%96%87%E8%AF%8D%E5%BA%93%E8%A7%84%E5%88%99.js?v1.9.2-2025-01-26
// @run-at       document-end
...
(function (window, document, undefined) {
    ...
    function traverseNode(node) {
        // 跳过忽略
        if (
            I18N.conf.reIgnoreId.test(node.id) ||
            I18N.conf.reIgnoreClass.test(node.className) ||
            I18N.conf.reIgnoreTag.includes(node.tagName) ||
            (node.getAttribute &&
                I18N.conf.reIgnoreItemprop.test(node.getAttribute("itemprop")))
        ) {
            return;
        }
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            // 翻译时间元素
            if (
                ["RELATIVE-TIME", "TIME-AGO", "TIME", "LOCAL-TIME"].includes(
                    node.tagName,
                )
            ) {
                if (node.shadowRoot) {
            ...
            if (["INPUT", "TEXTAREA"].includes(node.tagName)) {
                ...
                if (
                    ...
                    /tooltipped/.test(node.className)
                ) {
    ...
    function getPage() {
        ...
        const analyticsLocation =
            (document.getElementsByName("analytics-location")[0] || {})
                .content || "";
        // 组织页
        const isOrganization =
            /\/<org-login>/.test(analyticsLocation) ||
            /^\/(?:orgs|organizations)/.test(pathname);
        // 仓库页
        const isRepository = /\/<user-name>\/<repo-name>/.test(
            analyticsLocation,
        );
    ...
    function fetchTranslatedText(key) {
        ...
        // 正则翻译
        if (enable_RegExp) {
            let res = (I18N[lang][page].regexp || []).concat(
                I18N[lang]["public"].regexp || [],
            ); // 正则数组
            ...
            onload: (res) => {
                try {
                    ...
                    const translatedText =
                        status === 200
                            ? response.biz[0].translateResult
                            : "翻译失败";
    ...
    function transBySelector() {
        ...
        let res = (I18N[lang][page]?.selector || []).concat(
            I18N[lang]["public"].selector || [],
        ); // 数组
