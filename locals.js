...
I18N.zh["settings/two_factor_authentication"] = I18N.zh["settings/security"];
I18N.zh["settings/auth"] = I18N.zh["settings/security"];
...
I18N.zh["settings/codespaces"] = { // 设置 - 代码空间
    "static": { // 静态翻译
        ...
        // 代码空间 https://github.com/settings/codespaces
            ...
            "Region": "地区",
            "Your default region will be used to designate compute resources to your codespaces. GitHub can set your region automatically based on your location, or you can set it yourself. Codespaces are deployed to a subset of Azure regions.": "您的默认区域将被用来为你的代码空间指定计算资源。 GitHub 可以根据你的位置自动设置您的区域，您也可以自己设置。代码空间部署到 Azure 区域的子集。",
            "Set automatically": "自动设置",
            "We will determine the closest available region based on your location (IP address) at codespace creation time.": "我们将在创建代码空间时根据你的位置（IP地址）确定最近的可用区域。",
            "Set manually": "手动设置",
...
I18N.zh["repository/pull"] = { // 仓库 - 某个拉取请求页面
    "static": { // 静态翻译
        ...
        // 某条具体的拉取请求 /<user-name>/<repo-name>/pull/<id> >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            "Merge without waiting for requirements to be met (bypass branch protections)": "合并时不需要等待要求得到满足（绕过分支保护）。",
...
I18N.zh["repository/blob"] = { // 仓库 - 新建议题页面
    "static": { // 静态翻译
        ...I18N.zh["repository-public"]["static"],
...
I18N.zh["repository/security"] = { // 仓库 - 安全页面
    "static": { // 静态翻译
        ...
        // 安全标签卡 & 安全概述 /<user-name>/<repo-name>/security
            ...
            "Private vulnerability reporting •": "私下漏洞报告 •",
                "Allow users to privately report potential security vulnerabilties": "允许用户私下报告潜在的安全漏洞",
        ...
        // 添加钩子 页面 /organizations/<org-login>/settings/hooks/new 同仓库添加钩子 页面 /<user-name>/<repo-name>/settings/hooks/new ====================================
            ...
            "Which events would you like to trigger this webhook?": "您希望哪些事件触发此 Web 钩子？",
                ...
                "Let me select individual events.": "让我选择单个事件。",
                    ...
                    // "": "拉取请求",
                        "Pull request assigned, auto merge disabled, auto merge enabled, closed, converted to draft, demilestoned, dequeued, edited, enqueued, labeled, locked, milestoned, opened, ready for review, reopened, review request removed, review requested, synchronized, unassigned, unlabeled, or unlocked.": "拉动请求的分配、自动合并禁用、自动合并启用、关闭、转换为草稿、删除、撤销排队、编辑、排队、标记、锁定、里程碑、打开、准备好审查、重新打开、删除审查请求、请求审查、同步、撤销分配、撤销标记或解锁。",
                    "Pushes": "推送",
