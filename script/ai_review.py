...
def main() -> None:
    ...
    if prior_issues:
        prior_block = (
            ...
            "- 【折叠用户已拒绝的意见】：若上次某条意见已被用户明确拒绝（在 PR 评论区/对话中以『不采纳』『不同意』『没必要』『NO』『拒绝』等明确表达），本次审查中该意见若仍存在，请放入 Markdown 折叠区块（<details>）内并标注『（用户已拒绝，仅存档参考）』，不要作为重点问题在正文突出列出。\n"
...
{{
  "summary": "完整的 Markdown 审查报告：## 概览 / ## 发现的问题（按 🔴 阻断 / 🟠 重要 / 🟡 建议 / 🔵 nit 分级，每条尽量给出 文件:行号）/ ## 优点 / ## 重写/改进建议（必要时）。用户明确拒绝过的意见，用 <details><summary>📌 用户已拒绝的意见（存档参考）</summary>...</details> 折叠区块收纳，不要混入正文重点问题。",
  "comments": [
    {{
      "path": "变更文件的路径",
      "line": 新文件中的行号（整数）,
